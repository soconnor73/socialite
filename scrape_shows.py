    ...
    if site_name == 'first_avenue':
        ...
        if os.path.exists(filepath):
            ...
            if html:
                ...
                time.sleep(1)
        if html:
            shows = parser.parse(html, start_date=start_date, end_date=end_date)
            print(f"Parsed {len(shows)} shows from trainride.org")
            all_shows.extend(shows)

    elif site_name == 'mn_state_fair_grandstand':

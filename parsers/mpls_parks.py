...
class MplsParksParser(BaseParser):
    """
    Parser for Minneapolis Parks and Recreation event calendar.
    URL: https://www.minneapolisparks.org/event-calendar/
    ...
    def _parse_api_event(self, ev: Dict) -> Optional[Dict[str, Any]]:
        ...
        if isinstance(venue_info, dict):
            venue = self._clean(venue_info.get('venue', ''))
        
        if not venue:
            venue = 'Minneapolis Parks'
